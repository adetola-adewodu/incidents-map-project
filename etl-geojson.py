...
def get_station_feature(features, data, index, coordinate_key):
...
def get_address_feature(features, data, i):
...
for file in dir_list:
    ...
    for i in data:
        if i == 'address':
            features = get_address_feature(features, data, i)
        
        if i == 'apparatus':            
            status = data[i][0]['unit_status']
            if status.get('acknowledged'):
                features = get_station_feature(features, data, i, 'acknowledged')

            if status.get('arrived'):
                features = get_station_feature(features, data, i, 'arrived')
            
            if status.get('available'):
                features = get_station_feature(features, data, i, 'available')

            if status.get('cleared'):
                features = get_station_feature(features, data, i, 'cleared')
            
            if status.get('dispatched'):
                features = get_station_feature(features, data, i, 'dispatched')

            if status.get('enroute'):
                features = get_station_feature(features, data, i, 'enroute')
            
            if status.get('~'):
                features = get_station_feature(features, data, i, '~')
